...
def classify_words(word_dict):
    """词语分类,找出情感词、否定词、程度副词"""
    # 读取情感字典文件
    sen_file = open('BosonNLP_sentiment_score\BosonNLP_sentiment_score\BosonNLP_sentiment_score.txt', 'r+', encoding='utf-8')
    # 获取字典文件内容
...
# 计算得分
def setiment_score():
    conment_file=open('评论汇总.txt','r+',encoding='utf-8');
    conment_list=[]
    score_list=[]
    conment_score_dir={}
    # 将字典写入到csv文件中
    for sentence in conment_file.readlines():
        conment_list.append(sentence.strip())
        #print(sentence.strip())
        # 1.对文档分词
        seg_list = seg_word(sentence.strip())
        ...
        #conment_score_dir[sentence]=score
    dataframe=pd.DataFrame({"评论内容":conment_list,"情感得分":score_list})
    dataframe.to_csv("results.csv",index=False,sep=',',encoding="utf-8" )

    #print(conment_score_dir)

# 测试
#print(setiment_score(str4))
